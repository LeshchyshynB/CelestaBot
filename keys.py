...
class KEY_MAPPING:
	def __init__(self):
		self.keyboardMapping = dict([(key, None) for key in p.KEY_NAMES])
	...
	def get_key(self, key):
		mods, vkCode = divmod(self.keyboardMapping[key], 0x100)
		return vkCode

	def press_button(self, button, delay):
		ctypes.windll.user32.keybd_event(button, 0, self.KEYEVENT_DOWN, 0)
		sleep(delay)
		ctypes.windll.user32.keybd_event(button, 0, self.KEYEVENT_UP, 0)

	def hold_press(self, button_list, button, delay):
		for i in button_list:
			ctypes.windll.user32.keybd_event(i, 0, self.KEYEVENT_DOWN, 0)
		self.press_button(button, delay)
		for i in button_list:
			ctypes.windll.user32.keybd_event(i, 0, self.KEYEVENT_UP, 0)